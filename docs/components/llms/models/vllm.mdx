...
2. **Start vLLM server**:
   ...
   # For embeddings (on a different port)
...
## Using vLLM Embeddings

You can also use vLLM for embeddings by running an embedding model on a separate port:
...
m = Memory.from_config(config)
m.add("I love programming with Python", user_id="alice")
